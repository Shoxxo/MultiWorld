package de.theneotv.multiworld.files;
...
public class FileCheckWorld {
    ...
    public static void checkVanillaW() {
        ...
        if (w != null) {
            ...
            try {
                st = conn.prepareStatement("SELECT worldname FROM worlds WHERE worldname = '" + w.getName() + "'");
                rs = st.executeQuery();
                ...
                if (!(rs.isBeforeFirst())) {
                    ...
                    Bukkit.getConsoleSender().sendMessage(
                            plugin.getPrefix() + " §c" + w.getName() + " befindet sich schon in der Datenbank.");
                }
        ...
        if (n != null) {
            ...
            Connection conn = sql.getConnection();
            try {
                st = conn.prepareStatement("SELECT worldname FROM worlds WHERE worldname = '" + n.getName() + "'");
                rs = st.executeQuery();
                ...
                if (!(rs.isBeforeFirst())) {
                    ...
                    Bukkit.getConsoleSender().sendMessage(
                            plugin.getPrefix() + " §c" + n.getName() + " befindet sich schon in der Datenbank.");
                }
            } catch (SQLException e1) {
                e1.printStackTrace();
            }
        ...
        if (e != null) {
            ...
            Connection conn = sql.getConnection();
            try {
                st = conn.prepareStatement("SELECT worldname FROM worlds WHERE worldname = '" + e.getName() + "'");
                rs = st.executeQuery();
                ...
                if (!(rs.isBeforeFirst())) {
                    ...
                    Bukkit.getConsoleSender().sendMessage(
                            plugin.getPrefix() + " §c" + e.getName() + " befindet sich schon in der Datenbank.");
                }
            } catch (SQLException e1) {
                e1.printStackTrace();
            }
