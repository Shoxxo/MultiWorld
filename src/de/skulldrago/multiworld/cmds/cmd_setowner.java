                    ...
                    if (target != null) {
                        ...
                        try {
                            ...
                            if (rs.next()) {
                                ...
                                if (owner.equals(p.getName()) || p.hasPermission("Multiworld.all")) {
                                    ...
                                    st3 = conn.prepareStatement(
                                            "SELECT numbers, max, vnumbers, vmax FROM worldplayers WHERE name = '"
                                                    + target.getName() + "'");
                                    ...
                                    if (rs3.next()) {
                                        ...
                                        if (typ.equals("void") && (!(voids == maxvoids))) {
                                            vwrong = false;
                                        } else {
                                            vwrong = true;
                                        }

                                        if (!(typ.equals("void") && welten == maxwelten)) {
                                            wrong = false;
                                        } else {
                                            wrong = true;
                                        }
                                        ...
                                        if (!(wrong) || (vwrong)) {
                                            ...
                                            if (residents.contains("nobody")) {
                                                ...
                                                sql.queryUpdate("DELETE FROM worldresidents WHERE resident ='"
                                                        + resident + "' AND worldname = '" + w.getName() + "'");
                                            }

                                            if (residents.contains(target.getName())) {
                                                residents.remove(target.getName());
                                                sql.queryUpdate("DELETE FROM worldresidents WHERE resident='"
                                                        + target.getName() + "' AND worldname='" + w.getName() + "'");
                                            }
                                            residents.add(p.getName());

                                            sql.queryUpdate("UPDATE worlds SET owner='" + target.getName()
                                                    + "' WHERE worldname='" + w.getName() + "'");
                                            sql.queryUpdate(
                                                    "INSERT INTO worldresidents (worldname, type, resident) VALUES ('"
                                                            + w.getName() + "', '" + rs.getString("type") + "', '"
                                                            + p.getName() + "')");

                                            if (typ.equals("void")) {

                                                voids = voids + 1;
                                                sql.queryUpdate("UPDATE worldplayers SET vnumbers = '" + voids
                                                        + "' WHERE name = '" + target.getName() + "'");
                                    ...
                                    if (rs3.isBeforeFirst()) {
                                        ...
                                        if (!(wrong) || (vwrong)) {
                                            ...
                                            if (typ.equals("void")) {
                                                ...
                                                welten = welten + 1;
                                                sql.queryUpdate("UPDATE worldplayers SET anzahl = '" + welten
                                                        + "' WHERE Name = '" + target.getName() + "'");
                            ...
                            if (cfg2.contains("Commands.Setowner.Error")) {
                                ...
                                p.sendMessage(prefix + " §cVorgang konnte nicht durchgeführt werden.");
                            }
                        }
                    } else {
