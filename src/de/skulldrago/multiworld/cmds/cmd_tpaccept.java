                        ...
                        if (player != null) {
                            ...
                            if (target != null) {
                                ...
                                st2 = conn.prepareStatement(
                                        "SELECT owner, locked FROM worlds WHERE worldname='" + w.getName() + "'");
                                ...
                                if (rs2.next()) {
                                    ...
                                    st3 = conn.prepareStatement("SELECT resident FROM worldresidents WHERE worldname='"
                                            + w.getName() + "'");
                                    ...
                                    if (Locked.equals("true")) {
                                        if (owner.equals(target.getName()) || residents.contains(target.getName())) {
                                            ...
                                            if (cfg2.contains("Commands.Tpaccept.Locked")) {
                                                ...
                                                p.sendMessage(
                                                        "§cDeine Welt ist gelockt, um den Spieler in deine Welt zu teleportieren entsperre deine Welt mit /unlock.");
                                            }
                                        }
                                    } else {
                                        target.teleport(p);
                                        if (cfg2.contains("Commands.Tpaccept.Finish")) {
                                            ...
                                            p.sendMessage(
                                                    prefix + " §c" + target.getName() + " wurde zu dir teleportiert.");
                                        }
                                        if (cfg2.contains("Commands.Tpaccept.FinishPlayer")) {
                                            String msg = cfg2.getString("Commands.Tpaccept.FinishPlayer");
                                            msg = msg.replaceAll("&", "§");
                                        ...
                                        sql.queryUpdate(
                                                "DELETE FROM tprequests WHERE AnfrageName='" + target.getName() + "'");
                                    }
                                }
                            } else {
